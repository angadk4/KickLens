...
4. **Match scope excludes playoffs (V2 §3) — assumes the historical source can distinguish them.** **RESOLVED by spike T-001 (2026-07-05):** playoffs ARE in the file and ARE cleanly separable by the R1 date-window rule (Decision Day + 1 day; 2020 MIB-knockout window), verified by the 34-games-per-team invariant. Also confirmed (T-002 cross-check): the file's Date/Time is **UK-local**, not UTC. See `docs/spikes/E1-playoffs.md`.
...
9. **"API-Football/Highlightly … current season expected available" (V2 §10).** **RESOLVED by spike T-002 (2026-07-06), split verdict:** **API-Football free tier = NO-GO** for the live loop — free plans serve only seasons 2022–2024 (explicit plan error on 2025/2026 fixtures/teams). **Highlightly free (BASIC) tier = GO** — current-2026 MLS fixtures, finalized results (verified against `USA.csv`), round labels, UTC kickoffs, stable match ids; 100 req/day. **The documented fallback is hereby adopted: Highlightly is the live primary; API-Football remains usable only as a historical (2022–2024) cross-check.** See `docs/spikes/E2-live-mls.md`.
